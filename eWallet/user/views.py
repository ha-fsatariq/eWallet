...
class RegistrationView(View):
    ...
    def post(self, request):
        ...
        if not User.objects.filter(Q(username=username) | Q(contact=contact) | Q(email=email)).exists():
            ...
            messages.error(request, 'Username , Contact or email is already registered')
            return render(request, self.template_name)
            ...
            if user.is_active:
                ...
                if user.check_password(password):
                    ...
                    if attempts > 5:
                        ...
                        messages.error(request,'Account locked. Please try again later after 5 minutes.')
    ...
    def post(self,request):
        ...
        if User.objects.filter(email=email).exists():
            if (password==confirmPassword):
                ...
                messages.error(request,'Please make sure that password and confirm password are same.')
                return render(request,'forgotPassword.html')
        else:
